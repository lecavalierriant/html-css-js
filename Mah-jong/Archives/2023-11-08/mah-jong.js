...
function cracher(tuile) {
	...
	if (tuileCracher.famille == "fleurs") {
		...
		document.getElementsByClassName("td-centre")[compteCracher].innerHTML = "<a ondblclick = piocherCentre(this) class = a-tuiles-centre id = a-tuile-crachee>" + tuileCracher.unicode + "</a>";
...
function piocherMur(afficherPioche) {
	...
	if (comptePiocherMur < 12) {
		if (comptePiocherMur == 10) {
			document.getElementsByClassName("td-mur")[Math.floor(compte1PiocherMur / 2)].innerHTML = "<a ondblclick = piocherMur() class = a-tuiles-mur id = a-bas-gauche>🀫</a>";
		...
		if (comptePiocherMur == 82) {
			document.getElementsByClassName("td-mur")[Math.floor(compte3PiocherMur / 2)].innerHTML = "<a ondblclick = piocherMur() class = a-tuiles-mur id = a-haut-droite>🀫</a>";
		} else if (comptePiocherMur % 2 == 0) {
...
function estPaire(tuile1, tuile2) { // pour mah-jong et simples paires de vents / dragons
...
function verouillageBoutons() {
	...
	if (basculeVerouillageBoutons) {
		document.getElementById("valider").style.opacity = 0.5;
		if (annonce != "Chow") {
		...
		document.getElementById("valider").style.opacity = 1;
		if (annonce != "Chow") {
...
function testerAnnonce() {

	tuilesMain = document.getElementsByClassName("a-tuiles-main");
	piocheMain = document.getElementById("a-pioche-main");
	dernierCrache = document.getElementById("a-tuile-crachee");
